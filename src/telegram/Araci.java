...
public class Araci implements CommandFactory {

    private static final Logger LOG = Logger.getLogger(Araci.class);

    @Override
    public Command createCommand(Message message, RequestHandler requestHandler) {
        System.out.println("Kişi: " + message.getChat().getId());
        return new Cevaplayici(message, requestHandler);
    }
}
